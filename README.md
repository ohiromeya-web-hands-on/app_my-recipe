...
- [Codex を VS Code で使う](./docs/codex-vscode.md)
...
- 2026-05-18: ローカル production build (`npx next build` + `next start`) で Lighthouse Performance 96 (`/`)
- 2026-05-18: Playwright + `@axe-core/playwright` で `/`, `/recipes`, `/recipes/[id]`, `/shopping` の violations=0
...
| `/api/export` | 認証必須の JSON エクスポート |
| `/api/upload/recipe-image` | 認証必須の Vercel Blob クライアント直送アップロードトークン発行 |
| `/api/cron/daily-maintenance` | cron secret で保護された日次メンテナンス |
