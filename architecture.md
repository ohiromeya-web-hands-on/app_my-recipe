...
Auth.js の Google Provider を利用し、サインイン時に Google の stable subject ID である `sub` を `OWNER_GOOGLE_SUB` と比較する。一致しないユーザーはセッションを発行しない。
...
必要な環境変数は `AUTH_SECRET`、`GOOGLE_CLIENT_ID`、`GOOGLE_CLIENT_SECRET`、`OWNER_GOOGLE_SUB` とする。
...
| `OWNER_GOOGLE_SUB` | 【コア】 | 書込可能な単一オーナーの Google `sub` |
